...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withString:(NSString *)string delegate:(id<PopoverViewDelegate>)delegate
{
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    [popoverView showAtPoint:point inView:view withCoreText:string];
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withCoreText:(NSString *)string
{
    ...
    NSMutableAttributedString *theStr=[[NSMutableAttributedString alloc] initWithString:string attributes:attrs];
    ...
    {
        if (paragraphRect.size.height>point.y)
        {
            height = point.y-10;
            isScroll=YES;
        }
    ...
    {
        if (paragraphRect.size.height>screenSize.height-point.y-80)
        {
            height = screenSize.height-point.y-90;
            isScroll=YES;
        }
        else
        {
    ...
    UITextView *textView=[[UITextView alloc] initWithFrame:CGRectMake(0, 0, screenSize.width - kHorizontalMargin*4.f, height)];
...
-(void)setupLayout:(CGPoint)point inView:(UIView*)view
{
    ...
    above = YES;
    if (topPoint.y - contentHeight >= CGRectGetMinY(topViewBounds))
    {
        //Position above.
        above = YES;
        
        boxFrame = CGRectMake(xOrigin, arrowPoint.y - contentHeight+10, boxWidth, boxHeight);
    }
    else{
        //Position below because it won't fit above.
        above = NO;
        
        boxFrame = CGRectMake(xOrigin, arrowPoint.y +60, boxWidth, boxHeight);
    }

    //NSLog(@"boxFrame:(%f,%f,%f,%f)", boxFrame.origin.x, boxFrame.origin.y, boxFrame.size.width, boxFrame.size.height);

    CGRect contentFrame = CGRectMake(boxFrame.origin.x + padding, boxFrame.origin.y+10, contentWidth, contentHeight);
...
- (void)drawRect:(CGRect)rect
{
    ...
    //// Shadow Declarations
    UIColor* shadow =BackGroundColor;
...
//    CGContextSetShadowWithColor(context, shadowOffset, shadowBlurRadius, [UIColor blueColor].CGColor);
//    CGContextBeginTransparencyLayer(context, NULL);
//    [popoverPath addClip];
//    CGContextDrawLinearGradient(context, gradient, CGPointMake(CGRectGetMidX(frame), CGRectGetMinY(frame) - topOffset), CGPointMake(CGRectGetMidX(frame), CGRectGetMaxY(frame) + 10), 0);
